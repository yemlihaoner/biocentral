...
class _BiocentralMainViewState extends State<BiocentralMainView>
  ...
  bool _checkForUpdatedPlugins(
    BiocentralPluginState? oldState,
    BiocentralPluginState newState,
  ) {
    if (oldState == null) {
      return true;
    }
    ...
    return !newState.pluginManager.activePlugins.every(
      (plugin) => oldState.pluginManager.activePlugins.contains(plugin),
    );
  }
    ...
    _tabController.addListener(() {
      ...
      if (!_tabController.indexIsChanging) {
        widget.eventBus.fire(
          BiocentralPluginTabSwitchedEvent(_tabs[_tabController.index]),
        );
      }
    });
  }
